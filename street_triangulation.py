...
def get_trunk_property(depthmap, detection, trunk_id, 
    ...
    # consider the bottom part of the image only 
    trunk = {}
    ...
    image_mid_row = depthmap.shape[0] / 2  # there is a bug here: the bottom of trunk may be above the image mid row. 
    image_mid_row = int(image_mid_row)
    bottom_part_row_idx = np.where(idx_row > image_mid_row)
    # box_height = bottom_row - top_row
    # trunk_bottom_part_cutoff_row =  top_row + int(box_height * 0.2)  # 20% of the box height
    # bottom_part_row_idx = np.where(idx_row > trunk_bottom_part_cutoff_row)
...
def form_image_pairs(detect_file, pano_meta, detect_dir=None):
    ...
    # print(direction_str)
    # if direction_str in ['L45', 'R135']:   # I make a mistake when naming the thumbnails: the "L135" and "L45" should be switched.
    if direction_str in ['L135', 'R135']:  # Correct
    ...
    if direction == "R":
        ...
        forward_R135_basename = link_f_panoId + "*R135.pkl.bz2"  # look behind from front neighbor
        ...
        if direction_angle == 90:
            ...
            # print("forward_R135_files for R90:", forward_R135_files)
    ...
    if direction == "L":
        ...
        forward_L135_basename = link_f_panoId + "*L135.pkl.bz2"
        # I make a mistake when naming the thumbnails: the "L135" and "L45" should be switched.
...
def get_trunks_in_an_image(detect_file, trunk_class_id, image_dir, pano_meta_df, depthmap_dir, local_crs):
    ...
    json_file = os.path.join(image_dir, panoID + ".json")
    # print("json_file:", json_file)
    pano_obj = GSV_pano(crs_local=local_crs, json_file=json_file)
    # print("Processing pano_obj:", pano_obj)
    # print("heading_deg:", pano_obj.jdata['Projection']['pano_yaw_deg'], heading_deg)

    # pano_obj = GSV_pano(crs_local=local_crs,panoId=panoID )
    # print("Processing pano_obj:", pano_obj)
    # print("heading_deg:", pano_obj.jdata['Projection']['pano_yaw_deg'], heading_deg)
    ...
    # print("get_trunks_in_an_image(): direction_str:", direction_str)  # passed
    ...
    # print(trunk_ids)
    # print("trunk_ids:", basename, trunk_ids)
    ...
    for trunk_id in trunk_ids:
        # print("trunk_id:", basename, trunk_id)
        # print("passed before get_trunk_property()")
...
def form_trunk_pairs(raw_df, near_thres=3, camera_dis_thres=20):
    '''
    purpose: form trunk pairs for triangulation. 
    near_thres: meter unit
    camera_dis_thres: meter
    '''
    df = raw_df.query(f" depth_h_distance < {camera_dis_thres} ") #.sort_values("image_file")
    # print("df: \n", df)
    # df = df.sort_values(['depth_x', 'depth_y'])  # bug: when the trees are in a straight line torwards to eath or north, the results may be wrong.
    # nearest neighbor search
    # print("df[['depth_x', 'depth_y']]:", df[['depth_x', 'depth_y']], df[['depth_x', 'depth_y']].dtypes)
    trunk_xy_lists = []
    gb_df_list = []
    unique_image_files = df['image_file'].drop_duplicates()

    for file in unique_image_files:
        gb_df = df.query(f" image_file == '{file}' ")
    # for img, gb_df in df.groupby(['image_file']): # NOTE: order matters! 
        # print(img)
        gb_df_list.append(gb_df)
        trunk_xy_list = list(zip(gb_df['depth_x'],  gb_df['depth_y']))
        # print("gb_df:", gb_df)
        trunk_xy_lists.append(trunk_xy_list)

    # print("trunk_xy_lists:", trunk_xy_lists)


    pairs = find_nearest_pairs(trunk_xy_lists[0], trunk_xy_lists[1])
    # pairs: the returned index for EACH groupby df (gb_df)
    ...
    for pair in pairs:
        dist = pair[2]
        if dist < near_thres:
            pair_idx.append(pair[0])
            gb_df_index = pair[1]
            pair_idx.append(len(gb_df_list[0]) + gb_df_index)
            # restore the index of the all_trunk_df.
...
def show_detailed_images(trunk_pair_df, local_crs, image_dir, segmented_dir, save_dir):
    ...
    for idx, image in enumerate(images):
        try:
            ...
            axs[0, idx].set_title(basenames[idx][:-4], y=1.03, fontsize=10)
            axs[0, idx].axis('off')
            ...
            print("Error in show_detailed_images()", e)
            continue
...
def find_nearest_pairs(set1, set2):
    ...
    # Find nearest neighbor for each point in set1
    for i, point in enumerate(set1):
        if i in used_set1:
            continue
        dist, idx = tree.query(point)
        if idx not in used_set2:
